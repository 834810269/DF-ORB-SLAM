...
cv::Mat Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp)
{
	...
	//mbf = double((int(max/10))*10);
	//cout << endl << "baseline mult fx: " << mbf << endl;

	cv::Scalar mean;  //均值
	cv::Scalar stddev;  //标准差

	cv::meanStdDev( mImDepth(cv::Rect(30,30,mImDepth.cols-30,mImDepth.rows-30)), mean, stddev );  //计算均值和标准差
	double mean_pxl = mean.val[0];
	double stddev_pxl = stddev.val[0];
	//if(mImPre.empty()){
	mbf = mean_pxl+2*stddev_pxl;
	///}
	mThDepth = mean_pxl+stddev_pxl/mean_pxl;
	//cout << endl << "Depth Threshold (Close/Far Points): " << mThDepth << endl;
    //}
    ...
    //mImCur = mImGray;
    clahe->apply(mImGray,mImCur);// --wang

    Track();

    mImPre=mImCur.clone(); // --wang
    ...
    else
    {
        ...
        if(bOK)
        {
            ...
            for(int i=0; i<mCurrentFrame.N; i++)
            {
                ...
                if(pMP)
                    if(pMP->Observations()<1&&!pMP->isTried)
                    {
            ...
            if(NeedNewKeyFrame()){
                ...
                // Delete Mid-term points created by trigulation
                for(list<MapPoint*>::iterator lit = mlpTemporalPointsTri.begin(), lend =  mlpTemporalPointsTri.end(); lit!=lend; lit++)
                {
                    MapPoint* pMP = *lit;
                    delete pMP;
                }
        ...
        if(!mCurrentFrame.mpReferenceKF)
            mCurrentFrame.mpReferenceKF = mpReferenceKF;
        //cout<<" Mid-term points size: "<<mlpTemporalPointsTri.size()<<endl;
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    Optimizer::PoseOptimizationEM(&mCurrentFrame);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    int nmatches = matcher.SearchByLK(mCurrentFrame,mLastFrame,th,mSensor==System::MONOCULAR,mImPre,mImCur);
    //cout<<"Search by LK: "<< nmatches <<endl;
    // If few matches, uses a wider window search
    if(nmatches<20)
    {
        cout<<" !!!! Search by LK error"<<endl;
        fill(mCurrentFrame.mvpMapPoints.begin(),mCurrentFrame.mvpMapPoints.end(),static_cast<MapPoint*>(NULL));
        nmatches = matcher.SearchByProjection(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR);//SearchByLK(mCurrentFrame,mLastFrame,2*th,mSensor==System::MONOCULAR,mImPre,mImCur);//
    }
...
// Track with lk precompute and F decompose
bool Tracking::TrackWithLK(){
    ORBmatcher matcher(0.9,true);

    // Update last frame pose according to its reference keyframe
    // Create "visual odometry" points if in Localization Mode
    UpdateLastFrame();// todo: add trigulate points
    // todo: setpose by F decompose
    mCurrentFrame.SetPose(mVelocity*mLastFrame.mTcw);
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(!Tcw.empty())
            {
                ...
                int nGood = Optimizer::PoseOptimizationEM(&mCurrentFrame);
                ...
                if(nGood<50)
                {
                    ...
                    {
                        nGood = Optimizer::PoseOptimizationEM(&mCurrentFrame);
